...
import './dropDownScss/DropDown.scss'

const DropDown = () => {
  const [toggleDropdown, setToggleDropdown] = useState(false)
  const [selection, setSelection] = useState('Select Community')
  const [arrowDropdown, setArrowDropdown] = useState(['Data Analysis', 'Web Development', 'Product Design'])
            ...
            {arrowDropdown.map((options) => {
              return (
                <li
                  key={options}
                  onClick={() => {
                    setSelection(options === 'Select Community' ? 'Select Community' : options)
